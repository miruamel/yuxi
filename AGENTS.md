...
- `1f1539e` fix(knowledge): `--kb-stats` short-circuit dropped the `--kb-max-lines` cap — `config.parse` returned a struct with `kb_max_lines = null` on the `--kb-stats` branch (config.zig:130-131), so `knowledge.printStats` never reached its cap-echo line (knowledge.zig:192) and an operator who passed `--kb-max-lines` to the inspector saw the same output as one who didn't. The short-circuit now returns the parsed cap (bare form still defaults to 200, matching `--cache`/`--report`/`--record`). Root cause was a test that asserted on a nonexistent ledger, not a code defect.
- `7aa179f` test(main): assert `--kb-stats` echoes the `--kb-max-lines` cap — `knowledge.printStats` logs the bound it applied (knowledge.zig:192) so an operator can see it, but no test round-tripped it through the CLI. New assertion runs `--kb-stats --kb-max-lines=5` against the populated ledger and expects the `cap (--kb-max-lines): 5` line in stdout, exit 0.
- `f1a540a` test: cover the `--kb-stats` CLI path and the `assessHealth` `max_steps_exceeded` clause — two branches were reachable only by manual invocation, so a regression there shipped silently. `main --kb-stats` (main.zig:30-33) had no test at all; new CLI-shell test asserts both the no-ledger short-circuit and the populated-ledger path. `assessHealth`'s `max_steps_exceeded` clause (monitoring.zig:174-178) had the same gap as the `run_time_exceeded` clause closed earlier; new test sets `ctx.deploys=1` + `ctx.max_steps_exceeded=1` and asserts the verdict carries `max-steps exceeded` and NOT `no deploy`.
- `3b716e0` test(main): cover `--kb-stats` CLI inspector paths — no-ledger short-circuit (exit 0, `nothing to summarize`) and populated ledger (exit 0, all six category counts plus the latest line).
- `b5a7d89` test(main): cover `emitReportAndExit` temp-report path — `emitReportAndExit` writes a temp report and passes it to the health hook when the caller sets `--health-hook` WITHOUT `--report`; the existing hook test always supplied a `--report` path, so that branch was dead in tests. New test passes `report_path=null` + an unhealthy `TaskResult` and asserts the hook fires and the temp report is cleaned up afterwards.
- `d598a76` test(monitoring): cover `assessHealth` wall-time-exceeded clause — the `run_time_exceeded` verdict branch had no direct test; the existing unhealthy-cycle test sets every other counter but never `run_time_exceeded`. New test sets `ctx.deploys=1` + `ctx.run_time_exceeded=1` and asserts the verdict carries `wall-time exceeded` and NOT `no deploy`, proving the `--max-time` abort is a distinct health signal.
- `270a40f` docs(agents): record the §24/§28/§36/§41-E governance sweep in Recent cycles (§14).
- `bcdc7b7` docs: sync CLI flag list with `printHelp` (`--max-time`, `--max-attempts`).
- `f116675` docs(changelog): record `--max-time`/`--max-attempts` AGENTS.md sync (§28).
- `e17b301` docs(changelog): record `--max-time` in-loop cap fix (§28).
- `1856d0a` docs(changelog): record `runlife.zig` clock-helper extraction (§8/§28).
- `8d587a2` refactor(core): extract `clockNs`/`wallClockExceeded` into `core/runlife.zig` (§8 SLOC cap). The `--max-time` in-loop cap fix pushed `engine.zig` to 209 SLOC, breaching the §8 ≤200 ceiling. The two clock helpers already belonged to `runlife.zig`'s domain (it owns the LAYER 7-9 tail and `flushRecord`); extracting them dropped `engine.zig` to 189 SLOC with no behavior change — every call site prefixed `runlife.`, and the in-loop regression test still passes. Same lesson as the earlier `finishRun`/`flushRecord` extraction: re-check the §8 SLOC ceiling after a feature lands, before it drifts over.
- `04cac94` fix(core): `--max-time` cap now checked inside the self-correction attempt loop. The wall-clock check ran once before the loop, so a single slow `evaluator.run` could blow past the operator's bound undetected. The shared `wallClockExceeded(ctx, start_ns)` helper now runs at the top of every attempt; a fast fake clock (`Ctx.clock_ns` seam, mirroring `llm_fn`) trips the per-attempt guard on the third tick while the pre-loop guard passes, so the new `plan_gate_test` regression test proves the cap aborts before any `buildstep` or `evaluator.run`. All 4 `plan_gate` tests pass; full suite green (`zig build test -j2`, 11 roots).
...
- `fc63c37` docs(changelog): Unreleased section now uses the §28 split format (🤖 Autonomous Agent Contributions vs Human / Other Contributions) — previously Unreleased was the only section written as a flat list, contradicting the provenance-split requirement.
- `ea886b7` docs(governance): sweep of every markdown command block found bare `zig build` / `zig build test` without the §8/§36 parallelism cap, contradicting the constraint already named in README.md:110 and CONTRIBUTING.md:35. Fixed in README, DESIGN, AGENTS, CONTRIBUTING, the PR template, and AUTONOMOUS_AGENT. Zig uses `-j2` (not `--jobs 2`, which is a GNU make convention and was already wrong in AUTONOMOUS_AGENT.md:68).
- `085a0ae` docs(governance): AGENTS.md Build & Run / Verify and AUTONOMOUS_AGENT.md repository invariants now carry `-j2` explicitly.
- `a9e5e85` docs(governance): CI build/test steps capped at `-j2` per the resource invariant.
- `8d61c87` docs(governance): CONTRIBUTING.md added (§24) and CHANGELOG Unreleased entries added under the §28 split format.
- `d3caa2c` docs(governance): `.github/PULL_REQUEST_TEMPLATE.md` added, embedding the mandatory autonomous-agent context and linking AUTONOMOUS_AGENT.md per §41-E.
