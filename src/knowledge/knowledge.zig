...
/// rejections, mock fallbacks, token-budget breaches, and the max-steps /
/// wall-clock autonomy-safety caps) so a later run that replays prior lessons
/// via `injectPrompt` sees *how* a prior cycle went, not just whether it
/// deployed. On a failed run with a captured evaluator error, the trimmed
/// error is appended (see `errSnippet`) so the next run also learns *why* it
/// failed — closing the learning loop for non-critic failures too, not only
/// for critic rejections (`recordCritic`).
    ...
    if (ctx.kb_path) |kb| {
        ...
        const lesson = if (err) |e|
            try std.fmt.allocPrint(
                ctx.allocator,
                "- {s}: {s} (steps={d} deploys={d} retries={d} critic_rej={d} mock_fb={d} budget_ex={d} max_steps_ex={d} run_time_ex={d}) error=\"{s}\"",
                .{ task, outcome, steps, ctx.deploys, ctx.retries, ctx.critic_rejections, ctx.mock_fallbacks, ctx.token_budgets_exceeded, ctx.max_steps_exceeded, ctx.run_time_exceeded, e },
            )
        else
            try std.fmt.allocPrint(
                ctx.allocator,
                "- {s}: {s} (steps={d} deploys={d} retries={d} critic_rej={d} mock_fb={d} budget_ex={d} max_steps_ex={d} run_time_ex={d})",
                .{ task, outcome, steps, ctx.deploys, ctx.retries, ctx.critic_rejections, ctx.mock_fallbacks, ctx.token_budgets_exceeded, ctx.max_steps_exceeded, ctx.run_time_exceeded },
            );
